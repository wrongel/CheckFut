...
/////////////////////////////// БРОПРАВКА 17.10.2017 //////////////////////////////
#ifdef BRO
	// Связь обработчика кнопки смены папки
	ON_BN_CLICKED(IDC_BUT_CHANGEFOL, OnButtonclick)
	// Нажатие на заголовок списка
...
BOOL CReportCtrl::SetColumnHeader(const CString& strHeadings)
{
	...
	for (int i = 0; i < aLong.GetSize(); i++)
	{
		...
		if (aShort.GetSize() > 0)
		{
			...
			/////////////////////////////// БРОПРАВКА 16.10.2017 //////////////////////////////
			#ifdef BRO
			
			// Занести в список номера столбцов с кнопками
			if (aShort.GetSize() > 3 && _ttoi(aShort[3]) == 1)
				m_ListButtHead.AddTail(i);

			// Исправление бага с выравниванием 1 столбца
			if (i == 0)
			{			
				bInserted |= (InsertColumn(GetColumnCount(), aShort[0], nFormat, WIDTH) >= 0);
				DeleteColumn(0);
...
BOOL CReportCtrl::SetSortable(BOOL bSet)
{
	...
	{
/////////////////////////////// БРОПРАВКА 23.10.2017 //////////////////////////////
	// При убирании стиля HDS_BUTTONS, не сделать выбор всех чекбоксов
	#ifndef BRO
		lStyle &= ~HDS_BUTTONS;
	#endif // !BRO
//////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////// БРОПРАВКА 24.10.2017 //////////////////////////////
#ifdef BRO
BOOL CReportCtrl::AssignList(CWnd* pParentWnd, UINT nID, CFont *pFont)
{
	if (pParentWnd == NULL) return 1;
	if (pParentWnd->m_hWnd == NULL) return 1;
	...
	if (pFont != NULL)
	{
		...
		pFont->GetLogFont(&zLogFont);
		if ((_tcslen(zLogFont.lfFaceName)) <= 2) return 2;
		pWnd->SetFont(pFont);
	}
...
//////////////////////////////////////////////////////////////////////////////////
void CReportCtrl::CheckBoxEnable(int nEnableFlag)
{
	m_nCheckEnable = nEnableFlag;
}
#endif
...
/////////////////////////////// БРОПРАВКА 17.10.2017 //////////////////////////////
#ifdef BRO
	// Создание кнопки, если был задан флаг столбца
	if (m_ListButtHead.Find(m_ptEditting.y, 0) != NULL)
	{
		m_ButChangeFol.Create(_T(".."), WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, CRect(rcEdit.right - 32, rcEdit.top - 1, rcEdit.right + 1,rcEdit.bottom + 1), this, IDC_BUT_CHANGEFOL);
		rcEdit.DeflateRect(0, 0, 32, 0);
	}
#endif
///////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////// БРОПРАВКА 17.10.2017 //////////////////////////////
#ifdef BRO
	m_ButChangeFol.DestroyWindow();			// Уничтожение кнопки
#endif
//////////////////////////////////////////////////////////////////////////////////
...
{
	m_headerImgList.Create(nBitmapID, 24, 2, crMask);
...
/////////////////////////////// БРОПРАВКА 17.10.2017 //////////////////////////////
#ifdef BRO
void CReportCtrl::OnButtonclick()
{
	CFileDialog *pDlg = new CFileDialog(TRUE, _T(".exe"), NULL, OFN_HIDEREADONLY, _T("Исполняемый файл(*.exe)|*.exe|Все файлы (*.*)|*.*||"), NULL); //TRUE=чтение, FALSE=запись
	pDlg->m_ofn.lpstrTitle = _T("Выберете исполняемый файл программы");

	if (pDlg->DoModal() == IDOK)
	{
		CFileFind Find;
		if (Find.FindFile(pDlg->GetPathName()) == 0)
			SetItemText(m_ptEditting.x, m_ptEditting.y, _T("Некоррекный путь"));
		else
			SetItemText(m_ptEditting.x, m_ptEditting.y, pDlg->GetPathName());
	}
	delete pDlg;
}
...
{
	if (((GetExtendedStyle() & LVS_EX_CHECKBOXES) != LVS_EX_CHECKBOXES) || m_nCheckEnable == 0)
	{
	...
	*pResult = 0;
	...
	pHead->SetItem(nItem, &hdItem);

	BOOL bl = hdItem.iImage == 1 ? TRUE : FALSE;
	int nCount = GetItemCount();
	for (nItem = 0; nItem < nCount; nItem++)
		ListView_SetCheckState(GetSafeHwnd(), nItem, bl);
}
#endif
/////////////////////////////////////////////////////////////////////////////
/////////////////////////////// БРОПРАВКА 07.03.2018 //////////////////////////////
#ifdef BRO
void CReportCtrl::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS* lpncsp)
{
	ShowScrollBar(SB_HORZ, 0);		// Убирание горизонтального скролла
