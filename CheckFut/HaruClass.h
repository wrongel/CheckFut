...
#define MmToPx 2.834647			// Константа перевода миллиметров в пиксели
...
protected:
	...
	HPDF_Doc m_Pdf;							// Текущий документ	
	HPDF_Page m_CurPage;					// Текущая страница
	HPDF_Font m_CurFont;					// Текущий шрифт
	HPDF_REAL m_CurTextHeight;				// Текущая высота шрифта
	HPDF_REAL m_CurTextLeading;				// Текущий межстрочный интервал
	HPDF_Point m_CurTextPoint;				// Текущая точка вывода текста в документ
	_HPDF_PageSizes m_PageSize;				// Текущий размер страницы
	_HPDF_PageDirection m_PageDirection;	// Текущая ориентация страницы

	// Текущие поля страницы, мм
	float m_nTopField;
	float m_nBottomField;
	float m_nLeftField;
	float m_nRightField;

	int m_nOneStrChar;			// Количество символов по ширине на одной странице
	int m_nNumberOfPages;		// Количество страниц в документе
	BOOL m_bNumPages;			// Флаг, нужна ли нумерация страниц в документе
	char m_sFooterText[256];	// Надпись в левом нижнем углу каждой страницы

	// Печать простого текста, который может переноситься на след строку средствами библиотеки
	char* AddSimpleText(char* sString, /*HPDF_TextAlignment*/ HPDF_UINT TextAlign);
	// Добавить в буфер строку длинной больше ширины страницы
	const int MultiString(const char* sStirng, char* sBuffer);
	// Проверка влезает ли блок на страницу по высоте, если нет, то создать новую страницу
	int CheckNewPageText(char* sBuffer, const int nStrings, /*HPDF_TextAlignment*/ HPDF_UINT TextAlign);
	// Добавить номер страницы
	int AddFooter();
	// Аналог AddString для занесения строки в ячейку таблицы
	int AddStringToTableCell(HPDF_REAL fLeft, HPDF_REAL fTop, HPDF_REAL fRight, HPDF_REAL fBottom, char* sText, /*HPDF_TextAlignment*/ HPDF_UINT TextAlign);			
	// Подсчет количества строк, на которые будет разделен текст. Необходим для корректности вертикального выравнивания																										
	int TextStringCount(char* sString, float nWidth);		
	// Корректировка ширины ячейки в зависимости от содержимого
...
public:
	...
	// Задание надписи в нижнем поле страниц и нумерации
	void SetFooter(BOOL bNumPages, char* sFooterText = nullptr);
	// Получить ширину текущей страницы
	float GetPageWidth(BOOL bWithFields);
	// Получить высоту текущей страницы
	float GetPageHeight(BOOL bWithFields);
	// Добавить новую страницу в конец документа, указатель в pCurPage
	int InitPage();
	// Добавить текст
	int AddString(char* sString, /*HPDF_TextAlignment*/ HPDF_UINT TextAlign = HPDF_TALIGN_LEFT);
	// Добавить текст CString
	int AddString(CStringA sCString, /*HPDF_TextAlignment*/ HPDF_UINT TextAlign = HPDF_TALIGN_LEFT);

	// Нарисовать и заполнить таблицу динамическим двумерным массивом
	int CreateTable(float nXpos, float nYpos, float nWidth, float nHeight, int nStrings, int nColumns, char** sTableText, /*HPDF_TextAlignment*/ HPDF_UINT TextAlign);
	// Нарисовать и заполнить таблицу статическим двумерным массивом
	int CreateTable(float nXpos, float nYpos, float nWidth, float nHeight, int nStrings, int nColumns, char* sTextArr, int nStringLenght,/*HPDF_TextAlignment*/ HPDF_UINT TextAlign);
	//int CreateTable(float nXpos, float nYpos, float nWidth, float nHeight, int nStrings, int nColumns, char* sTableText, int nCountStrings, int nStringLenght, /*HPDF_TextAlignment*/ HPDF_UINT TextAlign);
	// Нарисовать и заполнить таблицу массивом CStringArray
	...
	// Задать параметры доступа, пароль для владельца и юзера, флаги доступа
	...
	//int DrawTable(float nXpos, float nYpos, float nWidth, float nHeight, int nStrings, int nColumns);	// Нарисовать таблицу
	//int FillTable(char** sTableText);											// Заполнить таблицу динамическим двумерным массивом
	//int FillTable(char* sTableText, int nCountStrings, int nStringLenght);		// Заполнить таблицу статическим двумерным массивом
private:
	HPDF_New PDF_New;
